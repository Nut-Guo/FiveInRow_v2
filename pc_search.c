...
void get_best_block(InfoPOOL *opinfo_pool, uint8_t color, Board *board) {
	uint8_t imax = 0;
	uint8_t i = 0;
	int16_t left_max = 1;
	int16_t tmp_left = 0;
	...
	if (op_value_info_pool.info[0].value > INT16_MAX >> 1 &&
		...
		if (tmp_value < INT16_MAX >> 1) {
			...
			for (uint8_t i = 0; i < local_Round; i++) {
				...
				exist_value += local_value[i % 2].value[tmpp.x][tmpp.y] * ((player == i % 2) ? 1 : -1);
			}
...
Point search_point(Point last) {
	...
	for (uint8_t i = 0; i < Round; i++) {
		update_value_table(local_PieceOnBoard.record[i]);
